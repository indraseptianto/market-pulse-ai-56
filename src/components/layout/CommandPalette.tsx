    ...
    const run = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!controller.signal.aborted && arr.length > 0) setApiResults(arr);
        }
      } catch { /* ignore */ }
  ...
  return (
    <>
      ...
      >
        ...
        <kbd className="text-[10px] rounded bg-muted px-1.5 py-0.5">Ctrl+K</kbd>
      </Button>
      <Button variant="ghost" size="icon" onClick={() => setOpen(true)} className="md:hidden" aria-label="Search">
        <Search className="h-4 w-4" />
      </Button>

      <CommandDialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setInputValue(""); setApiResults([]); } }}>
        ...
        <CommandList>
          ...
          {hasQuery && displayResults.length === 0 && !loading && (
            <CommandEmpty>Tidak ada hasil untuk &quot;{inputValue}&quot;.</CommandEmpty>
          )}
